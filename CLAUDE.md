...
- `make -C test` — engine unit tests (blinker/block/glider/R-pentomino, loop detection, max-generation watchdog, population stagnation), 140 checks, UBSan by default (`make -C test test-asan` adds ASan).
...
- **Termination is three independent heuristics, OR'd by the orchestrator.** A run reseeds when *any* fire, and each is configured by one field of `EngineConfig` (set that field to 0 to disable it):
  - **`DetectLoop()` — exact recurrence (hash ring, `loopHistorySize`).** Stores one 32-bit hash per generation (not full frames), so a deep window is cheap: the sketch uses `LOOP_HISTORY` (32) and the host emulator 128, catching far longer oscillator periods than the original 4-frame ring — e.g. a glider's 100-generation torus recurrence. Trade-off: a ~2⁻³² chance a hash collision reports a loop one generation early (a harmless slightly-early reseed). It can't catch periods longer than the window — that's what the watchdog is for.
  - **`WatchdogExpired()` — max-generation backstop (issue #8, `maxGenerations`).** Counts committed generations (`generationCount`, reset to 1 by `Initialize`, +1 per `CommitNextGen`) and fires at the cap. Catches gliders/spaceships drifting around the torus and chaotic/long-period fields that never recur within the hash window. Firmware default 1000 (~8 min at 500 ms/step).
  - **`PopulationStagnant()` — settled-field heuristic (issue #9 tail, `populationWindow`).** Tracks the live-cell count in O(1) (`lastPopulation` + a `stagnantRun` run-length counter, not a window buffer) and fires once the population has been unchanged for `populationWindow` consecutive generations. Population is recorded in `ComputeNextGen` alongside the hash (only when enabled). Kept well above `loopHistorySize` (firmware default 150) so ordinary oscillators are caught by `DetectLoop` first.
- **`EngineConfig` is the engine's config struct** (a plain aggregate, brace-initialized: `EngineConfig{loopHistorySize, maxGenerations, populationWindow}`). A convenience `ConwayEngine(unsigned short loopHistorySize)` constructor builds one with both backstops disabled — used by the loop-detection-only unit tests.
- **The step/display state machine** lives in `GameOfLifeStep()` via the `displayStepCounter` flag: a normal step computes → checks `DetectLoop() || WatchdogExpired() || PopulationStagnant()` → either commits or flips the flag; the next tick shows the counter screen (`ShowCounterScreen`), reseeds (`SeedRandom()` → `srand(analogRead(A0))`), and restarts.
